...
hidden1 = tf.layers.dense(inputs=inputs, units=256, activation=tf.nn.leaky_relu)
...
with tf.Session() as sess:
        ...
        for epoch in range(epochs):
            ...
            if (epoch+1) % step == 0:
                cost = sess.run(tf.reduce_mean(c))
                train_accuracy = accuracy.eval(feed_dict={inputs: trainX, labels: trainY})
                print("Epoch: {}".format(epoch + 1), "cost={}".format(cost), "training accuracy %g"%(train_accuracy))
